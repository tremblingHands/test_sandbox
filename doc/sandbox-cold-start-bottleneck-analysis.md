...
├── cni.setup_pod_network ≈ 5.77s   (~49%)   ← bridge CNI
...
**profile 结论**：单次冷启动时间几乎被 **CNI bridge** 与 **shim 创建** 平分；metadata 单次不高，但 Update 次数多、高并发下会被锁放大。
...
| **`netlink_*` / `rtnetlink_*`** | 高 | 配网、路由、接口操作 |
...
**perf 结论**：与 profile 对齐——**bridge + iptables** 与 **shim** 是内核侧两大热点；大量时间处于 **futex 等待**（与 pprof mutex/block 一致）。
...
```
① CNI bridge + iptables（ipMasq）
   profile: cni.setup ≈ 49%
   perf:    xtables / iptables / bridge / netlink
   机制:    每沙箱写 CNI-* nat 链，xtables 全局锁，并发恶化
...
profile:  CNI 5.8s + NewTask 4.8s ≈ 11.7s
              │                    │
perf:     iptables/bridge      shim/runc/futex
              │                    │
pprof:    setupPodNetwork      mutex/DB + selectgo
              │                    │
...
- **换 Calico/Cilium eBPF** 可从根本上去掉这类 per-pod iptables，但需 Kubernetes 与完整数据面；对本机纯 crictl 压测，更直接的是：
  - `"ipMasq": false`
  - 或改用 **ipvlan**（仓库 `scripts/setup.sh` 已支持）
  - 或压测不需出网时用 hostNetwork 做对照
...
| 实验 | 目的 | 预期 |
|------|------|------|
| A. `ipMasq: false` | 去掉每沙箱 iptables | `cni.setup_pod_network` 明显下降 |
| B. CNI 改为 ipvlan | 去掉 bridge+masq 路径 | CNI 阶段下降，perf 中 iptables 热点消失 |
| C. workers 32 / 64 / 128 | 锁竞争曲线 | P95 随并发非线性上升则确认锁/漏斗 |
| D. containerd 核 4 → 16+ | 缓解 4 核漏斗 | containerd 饱和缓解，总吞吐上升 |
| E. 同配置加 `--profile` 对比 A/B | 量化阶段收益 | 看 bridge span 是否缩短 |
...
| 未上 K8s 就部署 Cilium/Calico | 成本高；本场景热点是 bridge `ipMasq`，不是 kube-proxy |
...
| 最大阶段 | **CNI bridge（~49%）** + **shim 创建（~41%）** |
| 用户态 | **metadata DB 锁 + bbolt 写** |
| 内核态 | **iptables/xtables + bridge/netlink + shim/futex** |
| 架构 | **containerd 4 核** 成为全局漏斗 |
| 非瓶颈 | 磁盘延迟、内存容量 |

**一句话**：本次瓶颈是「高并发 + bridge/ipMasq 的 iptables 串行 + metadata 全局锁 + shim 创建」在「containerd 4 核」上叠加；优先用 **关 ipMasq / 换 ipvlan** 与 **降并发或扩 containerd 核** 做对照验证。
